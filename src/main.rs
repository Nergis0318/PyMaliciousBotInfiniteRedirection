...
use warp::{Filter, http::Uri, reject::Reject};
...
async fn handle_redirect(full_path: warp::filters::path::FullPath) -> Result<impl warp::Reply, warp::Rejection> {
    let random_token = generate_random_token(8);
    let redirect_url = format!("/{}", random_token);
    println!("요청 수신: {} -> 리디렉션: {}", full_path.as_str(), redirect_url);
    
    // Create a Uri from the redirect url string - this should always work for our format
    let uri: Uri = redirect_url.parse().map_err(|_| warp::reject::custom(InvalidUri))?;
